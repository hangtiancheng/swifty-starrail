...
export function useAchievementFiltered(
  ...
  const filteredItemsUnsorted = useMemo(() => {
    ...
    if (filter.ShowHidden || filter.ShowVisible) {
      if (!filter.ShowVisible)
        items = items.filter((item) => item.achievement_show_type === "隐藏");
  ...
  const filteredItems = useMemo(() => {
    ...
    if (orderRef.current.length > 0) {
      const itemMap = new Map(items.map((i) => [i.achievement_id, i]));
      const stable: typeof items = [];
      ...
      orderRef.current = stable.map((i) => i.achievement_id);
      return stable;
    }

    orderRef.current = sorted.map((i) => i.achievement_id);
    return sorted;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filteredItemsUnsorted, sortKey]);
