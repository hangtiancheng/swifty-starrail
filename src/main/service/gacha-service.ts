    ...
    if (type === "srgf_v1.0" && !Array.isArray(uid)) {
      ...
      const result = await dialog.showSaveDialog(
        BrowserWindow.getAllWindows()[0],
        {
          ...
          defaultPath: join(
            app.getPath("desktop"),
            `lark-star-rail-gacha-export-v${app.getVersion()}-${this.uids[uid]}-${uid}.SRGF.json`,
          ),
    ...
    if (type === "uigf_v4.1") {
      ...
      const result = await dialog.showSaveDialog(
        BrowserWindow.getAllWindows()[0],
        {
          ...
          defaultPath: join(
            app.getPath("desktop"),
            `lark-star-rail-gacha-export-v${app.getVersion()}.UIGF.json`,
          ),
  ...
  async importData(
    ...
    if (type === "srgf_v1.0" && Object.keys(data).length === 0) {
      ...
      if (
        ...
        return { msg: "不支持的 SRGF 版本" };
      if (!(data as Record<string, unknown>).list) return { msg: "无数据" };
    }
    ...
    if (
      ...
      if (info?.version !== "v4.0" && info?.version !== "v4.1")
        return { msg: "不支持的 UIGF 版本" };

      const hkrpg = (data as Record<string, unknown[]>).hkrpg;
      if (!hkrpg || hkrpg.length === 0) return { msg: "无星铁数据" };

      let lastUid = "";
      for (const userNode of hkrpg as Record<string, unknown>[]) {
        if (!/^\d{9}$/.test(userNode.uid as string)) return { msg: "无效 UID" };
        const srgfData = {
